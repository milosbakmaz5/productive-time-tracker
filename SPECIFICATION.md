...
- **`AddEntryModal`** (`src/components/AddEntryModal.tsx`) — opened via a "+ Add time entry" button in the header, next to the settings menu (collapsing to just "+" below the `sm` breakpoint, via a `hidden`/`sm:inline` span pair rather than swapping the button's rendered content, so the accessible name stays "Add time entry" - set explicitly via `aria-label` - regardless of which span is visually shown); a modal rather than its own route, since only *editing* has the explicit "own route" requirement from the assignment. Defaults its date field to whichever date is currently selected on the list. On success, the list's selected date switches to match the created entry's actual date (from the server response, not the form's local state) and that date's query is invalidated - so the new entry is immediately visible even if it was created for a date other than the one currently being viewed.
  ...
  **Focus management**: `useFocusTrap` (`src/lib/useFocusTrap.ts`), shared by `Modal` and `ConfirmDialog`, does the three things a modal dialog needs and neither component did before it existed: moves focus into the dialog when it mounts, keeps Tab/Shift+Tab cycling within the dialog's own focusable elements instead of escaping to the page underneath, and restores focus to whatever element was focused right before the dialog opened once it unmounts - not just "back to the page," but the literal trigger. Verified directly: Shift+Tab from the first field correctly wraps to Cancel *specifically because* the disabled submit button is excluded from the tab order at that moment (re-queried live, not a stale snapshot from mount), and closing returns focus to whichever button opened the modal.

  Initial focus defaults to the first focusable element, which for a bare `Modal` (no `initialFocusRef`) is its own Close button - fine for `ConfirmDialog`, where landing on the safer of the two actions is the point, but not useful for the create/edit forms, where every open should start you typing a duration, not sitting on a button you'd rarely want first. `Modal` accepts an optional `initialFocusRef` to override this; `AddEntryModal` and `EditEntryPage`'s form both pass a ref to `DurationInput`'s underlying `<input>` (via `React.forwardRef` on `DurationInput`, threaded through `EntryFormFields`' `durationInputRef` prop) so opening either one - including landing on `/entries/:id/edit` directly via a refresh, not just via the "⋮" menu - focuses Duration first.

  **Known gap**: this relies on `document.activeElement` still being the trigger at the moment the dialog's mount effect runs. That holds whenever the trigger stays mounted (e.g. `Modal`'s own Close button triggering the nested "Discard changes?" `ConfirmDialog`) but not when the trigger unmounts as part of the same update that opens the dialog - deleting an entry via the "⋮" menu's Delete item closes that dropdown (unmounting Delete) in the same state update that opens the delete `ConfirmDialog`, so focus has already reverted to `<body>` by the time the effect captures "previously focused." Left as a known limitation rather than solved here, since fixing it needs the capture to happen earlier (in `EntryActionsMenu`, before the dropdown closes) rather than inside the dialog itself - a larger change than this pass's scope.
...
**Automated**: [Vitest](https://vitest.dev) unit tests for the app's pure logic modules - the parts with real branching complexity and no dependency on the DOM, network, or React (`npm run test`, 52 tests / 5 files):

- `duration.test.ts` - every branch of `parseDurationInput`'s magnitude rule (`<10` → hours, `>=10` whole → minutes, `>=10` fractional → hours), `HH:MM` parsing, the 24h cap, and a `formatAsHHMM` round-trip.
- `week.test.ts` - `startOfWeek`'s Monday-offset math including the `getDay()===0` (Sunday) special case, cross-month `weekRangeLabel` formatting, and `addDays` rolling over month/year boundaries.
- `html.test.ts` - `stripHtml`/`noteToHtml` bullet and paragraph handling, HTML-entity decoding, that embedded `<script>` content never executes (DOMParser results are detached), and a round-trip test (`stripHtml(noteToHtml(x)) === x`) that directly guards the note-corruption bug described in §6.
- `note.test.ts` - the empty-bullet-vs-real-content boundary `hasNoteContent`/`normalizeNoteForSubmit` are built around.
- `errors.test.ts` - `ApiError.fromResponse`'s JSON:API error-parsing branches (friendly-code override, field-prefixed detail, title fallback, generic fallback) and `getErrorMessage`.

These were chosen deliberately over UI/integration tests: small, dependency-free functions with the most non-obvious branching in the app, several of which had real bugs caught by hand during development (the note HTML round-trip, the exact duration magnitude cutoff) that a regression suite would have caught on the *next* change, not just this one.

**Manual, not automated**: every other feature was verified end-to-end against the real Productive API (the test account referenced throughout this document) as it was built - login/logout/session persistence across a refresh, create/list/edit/delete against real `time_entries` records, the `service_id` auto-resolution and its fallback path, loading/empty/error states (including a real offline check via DevTools' network throttle), keyboard/focus behavior on the modals, and the responsive/mobile layout at multiple viewport widths.

**Not covered at all**: component/hook tests (would need React Testing Library plus mocked TanStack Query - `useFocusTrap` and the inline note/duration editors would be the next candidates), the API-layer functions that wrap `fetch` (request shape, header, and error-path tests would need a mocked `fetch`), and end-to-end tests against a real or mocked API server. Judged out of proportion for this scope given the extent of manual verification already done.
