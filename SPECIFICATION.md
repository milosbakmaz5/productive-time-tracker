...
- **`WeekDayStrip`** (`src/components/WeekDayStrip.tsx`) - the seven day cells (each showing that day's total, e.g. `01:30`) plus a weekly-total cell, at the top of the page content below the header. Clicking a cell selects that day (the same `date` URL param `EntriesPage` already drives everything from).

The header (`EntriesPage`) is `sticky top-0` with an explicit `z-20` - above the entry actions dropdown (`z-10`, so an open dropdown never covers it) but below modals (`z-50`/`z-60`, so Add/Edit still overlays it) - so the week nav and day totals stay reachable without scrolling back up through a long entry list.
