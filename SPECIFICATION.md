...
Mirrors a pattern from Productive's own web app: above the entry list, `WeekDayStrip` (`src/components/WeekDayStrip.tsx`) shows the Monday–Sunday week containing the selected date as seven clickable day cells (each showing that day's total, e.g. `01:30`), a weekly-total cell, and prev/next-week arrows. Clicking a day cell selects it (same `date` URL param the native date input already drove); the arrows shift the selected date by ±7 days, keeping the same weekday selected across weeks rather than resetting to Monday. The native date input stays alongside it for jumping to an arbitrary date - Productive's own version pairs its day strip with a calendar-popover for that, which was scoped out here as a smaller follow-up since the plain date input already covers the same need.
